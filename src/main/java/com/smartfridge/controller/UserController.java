...
public class UserController 
{
    ...
    public ResponseEntity<?> login(@RequestBody UserLoginRequest loginRequest) 
    {
        ...
        if (optionalUser.isPresent()) 
        {
            ...
            user.setPassword(null); // Oculta la contraseña antes de responder
            return ResponseEntity.ok(user); // Respuesta 200 OK con los datos del usuario
        } else
        {
            return ResponseEntity.status(401).body("Credenciales inválidas"); // Error 401 si no se autentica
        }
    }
...
public ResponseEntity<?> register(@RequestBody UserRegisterRequest registerRequest) 
{
    try 
    {
        ...
        user.setPassword(null);  // Oculta la contraseña antes de devolver el usuario
        return ResponseEntity.ok(user); // Registro exitoso, devuelve usuario
